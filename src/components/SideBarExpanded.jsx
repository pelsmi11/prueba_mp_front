...
const ExpandedSidebar = ({ onLogoClick }) => {
                        ...
                        <div>
                            <ul className="px-4 pb-4 text-sm font-medium">
                                {
                                    navsFooter.map((item, idx) => (
                                        <li key={idx}>
                                            <Link to={item.href} onClick={item.onClick} className="flex items-center gap-x-2 text-gray-600 p-2 rounded-lg  hover:bg-gray-50 active:bg-gray-100 duration-150">
                                                <div className="text-gray-500">{item.icon}</div>
                                                {item.name}
                                            </Link>
                                        </li>
                            ...
                            <div className="py-4 px-4 border-t">
                                <div className="flex items-center gap-x-4">
                                    <img src="https://xsgames.co/randomusers/assets/avatars/pixel/10.jpg" className="w-12 h-12 rounded-full" />
                                    <div>
                                        <span className="block text-gray-700 text-sm font-semibold">{ user.email }</span>
                                        <Link
                                            to= {user.fiscal_id ? `/profile/${user.fiscal_id}`: user.user?.fiscal?.id ? `/profile/${user.user.fiscal.id}`:'/profile'}
